...
def main() -> int:
    ...
    run("保存本次原始数据快照和指纹", "snapshot_data.py")
    run("统一周度人民币数据", "build_unified_weekly.py")
    run("生成事实型周报数据", "generate_weekly_json.py")
    run("生成本地周报分析", "generate_weekly_analysis.py")
    run("发布前数据真实性复核（失败则阻止发布）", "validate_weekly_data.py")
    run("通过发布闸门后更新周报页面", "generate_weekly_html.py")
    print("\n周报更新完成。")
    return 0
