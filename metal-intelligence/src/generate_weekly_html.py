...
INPUT_PATH = ROOT / "data" / "processed" / "weekly_analysis.json"
...
def trend_chart(metal: str, points: list[dict]) -> str:
    ...
    <div class="trend-chart">
      <div class="chart-caption"><span>{html.escape(metal)} · 近3个月周均价趋势</span><strong class="{period_tone}">{pct(period_change)}</strong></div>
      ...
      <div class="trend-stats">
        ...
        <div><span>本周较上周均价</span><strong class="{'up' if latest_change is not None and latest_change > 0 else 'down' if latest_change is not None and latest_change < 0 else 'flat'}">{latest_change_text}</strong></div>
      </div>
    </div>
    """
...
def source_tag(entry: dict) -> str:
    observed = f" · {html.escape(entry['observed_date'])}" if entry.get("observed_date") else ""
    if entry.get("url"):
        return f'<span class="source-note">{html.escape(entry.get("source", "来源"))}{observed} · <a href="{html.escape(entry["url"])}" target="_blank" rel="noreferrer">查看来源</a></span>'
    return f'<span class="source-note">{html.escape(entry.get("source", "内部复核"))}{observed}</span>'


def price_story(item: dict, points: list[dict]) -> str:
    """Create the factual price context shown before qualitative explanations."""
    change = item["weekly_change_pct"]
    direction = "上涨" if change > 0 else "下跌" if change < 0 else "持平"
    values = [point["price"] for point in points if point.get("price") is not None]
    if len(values) >= 2 and values[0] != 0:
        trend_change = (values[-1] / values[0] - 1) * 100
        trend_direction = "上涨" if trend_change > 0 else "下跌" if trend_change < 0 else "基本持平"
        period = f"近{len(values)}周"
        return f"本周较上周均价{direction} {abs(change):.2f}%；从{period}看，累计{trend_direction} {abs(trend_change):.2f}%。"
    return f"本周较上周均价{direction} {abs(change):.2f}%；中期趋势数据暂不足。"


def evidence_badge(level: str) -> str:
    return f'<span class="evidence-badge evidence-{html.escape(level)}">证据：{html.escape(level)}</span>'


def release_gate(allow_unverified: bool = False) -> None:
    """Block standalone publication unless the deterministic review passed."""
    if allow_unverified:
        return
    if not REVIEW_PATH.exists():
        raise RuntimeError("发布闸门未通过：缺少 reports/weekly_data_review.json")
    review = json.loads(REVIEW_PATH.read_text(encoding="utf-8"))
    if review.get("status") != "通过":
        failed = [item["name"] for item in review.get("checks", []) if item.get("status") != "通过"]
        raise RuntimeError("发布闸门未通过：" + "、".join(failed))


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the weekly report after data review")
    parser.add_argument("--allow-unverified", action="store_true", help="仅生成内部预览，跳过发布闸门")
    args = parser.parse_args(argv)
    release_gate(args.allow_unverified)
    report = json.loads(INPUT_PATH.read_text(encoding="utf-8"))
    facts = json.loads(FACTS_PATH.read_text(encoding="utf-8"))
    driver_document = json.loads(DRIVERS_PATH.read_text(encoding="utf-8"))
    drivers = driver_document.get("metals", {})
    ...
    for item in report["metal_analysis"]:
        ...
        driver = drivers.get(item["metal"], {"headline": "暂未提供价格解读", "factors": [], "evidence_gaps": [], "caveat": "", "conclusion": "暂不足以下结论。", "conclusion_status": "insufficient"})
        factor_blocks = "".join(f"""
          <li class="factor-item">
            <div class="factor-head"><strong>{html.escape(entry.get('factor', '影响因素'))}</strong>{evidence_badge(entry.get('evidence_level', '不足'))}</div>
            <p>{html.escape(entry.get('plain_explanation', '暂未提供解释。'))}</p>
            <div class="factor-effect"><span>可能影响</span><strong>{html.escape(entry.get('possible_effect', '暂不确定'))}</strong></div>
            <div class="factor-source">{source_tag(entry)}</div>
            <div class="factor-limit">限制：{html.escape(entry.get('limitation', '暂无限制说明。'))}</div>
          </li>
        """ for entry in driver.get("factors", []))
        gaps = "".join(f"<li>{html.escape(gap)}</li>" for gap in driver.get("evidence_gaps", []))
        watch = "".join(f"<li>{html.escape(item)}</li>" for item in driver.get("watch_next_week", []))
        status_labels = {"supported": "线索较清晰", "mixed": "因素同时存在", "weak": "仍偏不确定", "insufficient": "证据不足"}
        status_label = status_labels.get(driver.get("conclusion_status"), "需继续观察")
        cards.append(f"""
        ...
        <article class="driver-card">
          <div class="driver-card-head"><div><strong>{html.escape(item['metal'])}</strong><span class="status-label">{html.escape(status_label)}</span></div><span>{html.escape(driver['headline'])}</span></div>
          <div class="reader-block"><strong>发生了什么</strong><p>{html.escape(price_story(item, points))} {html.escape(driver.get('reader_summary', ''))}</p></div>
          <div class="reader-block"><strong>可能的影响因素</strong><ul class="factor-list">{factor_blocks or '<li>目前没有足够资料支持具体解释。</li>'}</ul></div>
          <div class="reader-block"><strong>目前结论</strong><p>{html.escape(driver.get('conclusion', '暂不足以下结论。'))}</p></div>
          <div class="reader-block"><strong>还缺什么证据</strong><ul class="gap-list">{gaps or '<li>暂未记录额外证据缺口。</li>'}</ul></div>
          <div class="reader-block"><strong>下周观察</strong><ul class="watch-list">{watch or '<li>暂未提供下周观察事项。</li>'}</ul></div>
          <p class="driver-caveat">说明：{html.escape(driver.get('caveat', '本节只提供可能解释，不代表确定因果。'))}</p>
        </article>
        """)
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .driver-card-head {{ display:flex; justify-content:space-between; gap:10px; align-items:start; }}
    .driver-card-head strong {{ display:block; font-size:17px; }} .driver-card-head > span {{ color:var(--muted); font-size:12px; text-align:right; }}
    .status-label {{ display:inline-block; margin-top:3px; color:var(--accent); font-size:11px; font-weight:600; }}
    .reader-block {{ margin-top:12px; padding-top:10px; border-top:1px solid var(--line); font-size:12px; line-height:1.6; }}
    .reader-block > strong {{ color:var(--accent); font-size:11px; }}
    .reader-block p {{ margin:4px 0 0; color:#5d5952; }}
    .factor-list, .gap-list, .watch-list {{ margin:5px 0 0; padding-left:17px; color:#5d5952; font-size:12px; }}
    .factor-list li, .gap-list li, .watch-list li {{ margin:8px 0; }}
    .factor-head {{ display:flex; justify-content:space-between; gap:8px; align-items:center; }}
    .factor-head strong {{ color:var(--ink); }}
    .evidence-badge {{ padding:2px 6px; border-radius:999px; background:#f3eee4; color:#76551f; font-size:10px; white-space:nowrap; }}
    .evidence-较强 {{ background:#e4f1e8; color:#2f7d5a; }} .evidence-中等 {{ background:#f3eee4; color:#76551f; }} .evidence-较弱, .evidence-不足 {{ background:#f3e5e3; color:#9b4e44; }}
    .factor-item p {{ margin:4px 0; }}
    .factor-effect {{ display:flex; gap:6px; align-items:baseline; color:var(--muted); font-size:11px; }}
    .factor-effect strong {{ color:var(--ink); font-size:11px; }}
    .factor-source {{ margin-top:3px; font-size:11px; }}
    .factor-source a {{ color:var(--accent); white-space:nowrap; }}
    .source-note {{ color:var(--muted); }}
    .factor-limit {{ margin-top:4px; color:var(--muted); font-size:11px; }}
    .driver-caveat {{ margin:10px 0 0; color:var(--muted); font-size:11px; }}
  ...
  <main>
    ...
    <section aria-labelledby="summary-title">
      ...
      <div class="rank-grid">
        ...
        <div class="rank"><small>本周涨幅最小</small><strong class="{ 'up' if snapshot['biggest_loser']['change_pct'] > 0 else 'down' if snapshot['biggest_loser']['change_pct'] < 0 else 'flat' }">{html.escape(snapshot['biggest_loser']['metal'])} {pct(snapshot['biggest_loser']['change_pct'])}</strong></div>
      </div>
    </section>
    <section aria-labelledby="prices-title">
      ...
      <p class="analysis-intro">价格卡片显示本周较上周均价变化；趋势图显示近3个月周均价变化。价格位置表示当前价格在近3个月区间中的位置：0% 接近最低价，100% 接近最高价。</p>
    ...
    <section aria-labelledby="drivers-title">
      <div class="section-head"><h2 id="drivers-title" class="section-title">本周价格解读</h2><span class="section-meta">事实、线索与限制</span></div>
      <p class="analysis-intro">先看本周价格发生了什么，再看可能的影响因素。影响因素是有来源的分析线索，不代表已经证明的单一原因。</p>
